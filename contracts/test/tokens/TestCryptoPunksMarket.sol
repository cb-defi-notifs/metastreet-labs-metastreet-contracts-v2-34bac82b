...
contract TestCryptoPunksMarket {
    ...
    function setInitialOwner(address to, uint punkIndex) public {
        if (msg.sender != owner) revert("Revert");
        if (allPunksAssigned) revert("Revert");
        if (punkIndex >= 10000) revert("Revert");
    ...
    function getPunk(uint punkIndex) public {
        if (!allPunksAssigned) revert("Revert");
        if (punksRemainingToAssign == 0) revert("Revert");
        if (punkIndexToAddress[punkIndex] != address(0)) revert("Revert");
        if (punkIndex >= 10000) revert("Revert");
    ...
    function transferPunk(address to, uint punkIndex) public {
        if (!allPunksAssigned) revert("Revert");
        if (punkIndexToAddress[punkIndex] != msg.sender) revert("Revert");
        if (punkIndex >= 10000) revert("Revert");
    ...
    function buyPunk(uint punkIndex) public payable {
        ...
        if (punkIndex >= 10000) revert("Revert");
        if (!offer.isForSale) revert("Revert"); // punk not actually for sale
        if (offer.onlySellTo != address(0) && offer.onlySellTo != msg.sender) revert("Revert"); // punk not supposed to be sold to this user
        if (msg.value < offer.minValue) revert("Revert"); // Didn't send enough ETH
        if (offer.seller != punkIndexToAddress[punkIndex]) revert("Revert"); // Seller no longer owner of punk
    ...
    function enterBidForPunk(uint punkIndex) public payable {
        ...
        if (punkIndexToAddress[punkIndex] == address(0)) revert("Revert");
        if (punkIndexToAddress[punkIndex] == msg.sender) revert("Revert");
        if (msg.value == 0) revert("Revert");
        Bid memory existing = punkBids[punkIndex];
        if (msg.value <= existing.value) revert("Revert");
        if (existing.value > 0) {
    ...
    function acceptBidForPunk(uint punkIndex, uint minPrice) public {
        ...
        Bid memory bid = punkBids[punkIndex];
        if (bid.value == 0) revert("Revert");
        if (bid.value < minPrice) revert("Revert");
    ...
    function withdrawBidForPunk(uint punkIndex) public {
        ...
        if (punkIndexToAddress[punkIndex] == msg.sender) revert("Revert");
        Bid memory bid = punkBids[punkIndex];
        if (bid.bidder != msg.sender) revert("Revert");
