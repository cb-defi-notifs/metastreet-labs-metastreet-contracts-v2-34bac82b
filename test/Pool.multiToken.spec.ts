...
describe("Pool Multi Token", function () {
  ...
  before("deploy fixture", async () => {
    ...
    multiTokenCollateralWrapper = await multiTokenCollateralWrapperFactory.deploy();
    ...
    /* Approve multiToken to transfer NFT */
    await nft1.connect(accountBorrower).setApprovalForAll(multiTokenCollateralWrapper.address, true);

    /* Approve pool to transfer multiToken NFT */
    await multiTokenCollateralWrapper.connect(accountBorrower).setApprovalForAll(pool.address, true);
  });
  ...
  describe("getters", async function () {
    ...
    it("returns expected collateral wrappers", async function () {
      ...
      expect(collateralWrappers[0]).to.equal(multiTokenCollateralWrapper.address);
  ...
  async function createActiveMultiTokenLoan(
    principal: ethers.BigNumber,
    duration?: number = 30 * 86400
  ): Promise<[string, string, ethers.BigNumber, string]> {
    /* Mint multi token */
    const mintTx = await multiTokenCollateralWrapper
      .connect(accountBorrower)
      .mint(nft1.address, [123, 124, 125], [1, 2, 2]);
    const multiTokenTokenId = (await extractEvent(mintTx, multiTokenCollateralWrapper, "BatchMinted")).args.tokenId;
    const multiTokenData = (await extractEvent(mintTx, multiTokenCollateralWrapper, "BatchMinted")).args.encodedBatch;
    ...
    const borrowTx = await pool
      ...
      .borrow(
        ...
        ethers.utils.solidityPack(
          ["uint16", "uint16", "bytes"],
          [1, ethers.utils.hexDataLength(multiTokenData), multiTokenData]
        )
      );
    ...
    return [loanReceipt, loanReceiptHash, multiTokenTokenId, multiTokenData];
  }
  ...
  describe("#quoteRefinance", async function () {
    ...
    it("correctly quotes refinance payment and repayment at original multi token loan maturity with same principal", async function () {
      /* Create Loan */
      const [loanReceipt] = await createActiveMultiTokenLoan(FixedPoint.from("25"));
  ...
  describe("#borrow", async function () {
    ...
    it("originates multi token loan", async function () {
      ...
      /* Validate events */
      await expectEvent(mintTx, multiTokenCollateralWrapper, "Transfer", {
        from: ethers.constants.AddressZero,
        to: accountBorrower.address,
        tokenId: multiTokenTokenId,
      });

      await expectEvent(borrowTx, multiTokenCollateralWrapper, "Transfer", {
      ...
      expect(decodedLoanReceipt.collateralToken).to.equal(multiTokenCollateralWrapper.address);
      expect(decodedLoanReceipt.collateralTokenId).to.equal(multiTokenTokenId);
      expect(decodedLoanReceipt.collateralWrapperContextLen).to.equal(ethers.utils.hexDataLength(multiTokenData));
      expect(decodedLoanReceipt.collateralWrapperContext).to.equal(multiTokenData);
    ...
    it("originates multi token loan with delegation", async function () {
      ...
      const simulatedRepayment = await pool
        ...
        .callStatic.borrow(
          ...
          ethers.utils.solidityPack(
            ...
            [1, ethers.utils.hexDataLength(multiTokenData), multiTokenData, 3, 20, accountBorrower.address]
          )
        );
      ...
      await expectEvent(borrowTx, delegationRegistry, "DelegateForToken", {
        ...
        contract_: multiTokenCollateralWrapper.address,
        tokenId: multiTokenTokenId,
        value: true,
      });
      ...
      await expect(
        pool
          ...
          .borrow(
            ...
            FixedPoint.from("26"),
            await sourceLiquidity(FixedPoint.from("25"), 6),
            ethers.utils.solidityPack(["uint16", "uint16", "bytes"], [1, 20 + 31 * 3, multiTokenData])
          )
      ).to.be.reverted;
    });
  ...
  describe("#repay", async function () {
    ...
    it("repays multi token loan at maturity", async function () {
      const mintTx = await multiTokenCollateralWrapper.connect(accountBorrower).mint(nft1.address, [124, 125], [1, 2]);
      const multiTokenTokenId = (await extractEvent(mintTx, multiTokenCollateralWrapper, "BatchMinted")).args.tokenId;
      const multiTokenData = (await extractEvent(mintTx, multiTokenCollateralWrapper, "BatchMinted")).args.encodedBatch;

      expect(await multiTokenCollateralWrapper.ownerOf(multiTokenTokenId)).to.equal(accountBorrower.address);
      ...
      expect(await multiTokenCollateralWrapper.ownerOf(multiTokenTokenId)).to.equal(pool.address);

      const multiTokenLoanReceipt = (await extractEvent(borrowTx, pool, "LoanOriginated")).args.loanReceipt;
      const multiTokenLoanReceiptHash = (await extractEvent(borrowTx, pool, "LoanOriginated")).args.loanReceiptHash;

      /* Get decoded receipt */
      const decodedLoanReceipt = await loanReceiptLib.decode(multiTokenLoanReceipt);

      /* Repay */
      await helpers.time.setNextBlockTimestamp(decodedLoanReceipt.maturity.toNumber());
      const repayTx = await pool.connect(accountBorrower).repay(multiTokenLoanReceipt);
      ...
      await expectEvent(repayTx, multiTokenCollateralWrapper, "Transfer", {
        from: pool.address,
        to: accountBorrower.address,
        tokenId: multiTokenTokenId,
      });
      ...
      await expectEvent(repayTx, pool, "LoanRepaid", {
        loanReceiptHash: multiTokenLoanReceiptHash,
        repayment: decodedLoanReceipt.repayment,
      });

      /* Validate state */
      expect(await pool.loans(multiTokenLoanReceiptHash)).to.equal(2);
  ...
  describe("#refinance", async function () {
    ...
    let multiTokenTokenId: ethers.BigNumber;
    let multiTokenData: string;
    ...
    it("refinance multi token loan at maturity with admin fee and same principal", async function () {
      ...
      /* Create Loan */
      [loanReceipt, loanReceiptHash, multiTokenTokenId] = await createActiveMultiTokenLoan(FixedPoint.from("25"));
      ...
      expect(decodedNewLoanReceipt.collateralToken).to.equal(multiTokenCollateralWrapper.address);
      expect(decodedNewLoanReceipt.collateralTokenId).to.equal(multiTokenTokenId);
    ...
    it("multi token loan fails on borrow and refinance in same block with same loan receipt fields", async function () {
      ...
      await setupLiquidity();
      pool.setAdminFeeRate(500);
      [loanReceipt, loanReceiptHash, multiTokenTokenId, multiTokenData] = await createActiveMultiTokenLoan(
        FixedPoint.from("25")
      );
      ...
      nodeReceipt.collateralToken = multiTokenCollateralWrapper.address;
    ...
    it("multi token loan fails on invalid caller", async function () {
      ...
      await setupLiquidity();
      pool.setAdminFeeRate(500);
      [loanReceipt, loanReceiptHash] = await createActiveMultiTokenLoan(FixedPoint.from("25"));
  ...
  describe("#liquidate", async function () {
    ...
    it("liquidates expired multi token loan", async function () {
      ...
      /* Validate events */
      await expectEvent(liquidateTx, multiTokenCollateralWrapper, "Transfer", {
        from: pool.address,
